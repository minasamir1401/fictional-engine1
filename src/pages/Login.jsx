      ...
      <div className="w-full max-w-md z-10 space-y-8 animate-in fade-in zoom-in duration-500">
        <div className="text-center space-y-2">
          <div className="inline-flex items-center justify-center w-20 h-20 rounded-3xl bg-slate-900 border border-slate-800 mb-4 shadow-2xl relative group">
            ...
            <LogIn className="text-primary relative" size={40} />
          </div>
          <h1 className="text-4xl font-black tracking-tight text-white">تسجيل الدخول</h1>
          <p className="text-slate-400 font-medium">مرحباً بك مجدداً في ويبينار سبورتس</p>
        </div>
            ...
            <div className="space-y-6">
              <div className="space-y-2 group">
                ...
                <input
                  ...
                  className="w-full bg-slate-950/50 border border-slate-800 rounded-xl py-4 px-4 text-slate-100 outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all text-left font-bold"
                />
              </div>

              <div className="space-y-2 group">
                <div className="flex justify-between items-center px-1">
                  <label className="text-sm font-bold text-slate-400 mr-1 flex items-center gap-2">
                    <Lock size={14} className="text-primary" /> كلمة المرور
                  </label>
                  <button type="button" onClick={() => alert('تواصل مع الإدارة لاستعادة كلمة المرور')} className="text-primary text-xs font-bold hover:underline">نسيت كلمة المرور؟</button>
                </div>
                <input
                  type="password"
                  required
        ...
        <div className="text-center space-y-6">
          <p className="text-slate-500 font-bold">
            ليس لديك حساب؟{' '}
            <button onClick={() => navigate('/register')} className="text-primary hover:text-primary/80 transition-colors">
              إنشاء حساب جديد
            </button>
          </p>
          <div className="flex items-center justify-center gap-6">
            <a href="#" onClick={(e) => { e.preventDefault(); alert('جارٍ الاتصال بالدعم الفني...'); }} className="text-slate-600 hover:text-slate-400 text-sm font-bold transition-colors">اتصل بالإدارة</a>
            <div className="w-1.5 h-1.5 bg-slate-800 rounded-full"></div>
            <a href="#" className="text-slate-600 hover:text-slate-400 text-sm font-bold transition-colors">عن التطبيق</a>
          </div>
        </div>
      </div>
    </div>
